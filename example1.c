...
program* mutation1(program* parent, int nodeNum, int actionNum)
{
    ...
    else if(actionNum > 47 && actionNum < 50 )
    {
        ...
        switch (actionNum) {
            case 48:
                if (mnode->fixed != 1 && (mnode->treenode1 != NULL && mnode->treenode1->fixed == 0 && (mnode->treenode2 == NULL || mnode->treenode2->fixed == 0)))
                {
		    printf("%d", mnode->depth);
                    if(mnode->parent == NULL)    //mnode == new
                    {
		    ...
		    else  if(mnode->depth == 2 && mnode->numofstatements == 6 && mnode->treenode1->type == 3 && (mnode->type == 0 || mnode->type == 1))
		   {
			   printf("heng");
			   while(mnode->treenode1->type == 2)
			   {
			   	treenode* x1 = mnode->treenode1;
				free(mnode);
				treenode* mnode = x1;		
			   }
			   new1 = mnode->treenode1;
			   new1->treenode1->parent = mnode->parent;
			   if(mnode->parent->treenode1 == mnode)
				   mnode->parent->treenode1 = new1->treenode1;
			   else
				   mnode->parent->treenode2 = new1->treenode1;
		   }
		    else if(mnode->depth != 2 && mnode->numofstatements == 2 && mnode->treenode1->type == 3 && (mnode->type == 0 || mnode->type == 1))
		    {
			    printf("heng");
			     while(mnode->treenode1->type == 2)
			     {
